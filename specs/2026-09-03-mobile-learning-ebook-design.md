...
### 设计阶段的不确定性（已在实现阶段实测解决）

`vitepress-sidebar` 对**嵌套文件夹**的排序与标题机制，官方文档当时说明不够明确，
本设计因而一度采用「五大块内部扁平、不嵌套子目录」来规避。

**该不确定性已解决**（详见实现计划「实现期发现」F8）：实测确认文件夹的标题、链接、
排序都能从其 `index.md` 的 frontmatter 读取，只需开启两个选项：

- `useFolderTitleFromIndexFile: true` —— 文件夹标题取 `index.md` 的 `title`，
  不开启则显示英文目录名（slug）
- `useFolderLinkFromIndexFile: true` —— 文件夹可点击，链接到其 `index.md`

**当前约定**：块内默认为扁平（大多数章节只有一页，没必要开文件夹）；当一章需要
多个子页时（例如「真机与模拟器」下的「安装」），把该章变为同名文件夹、
原章节页移入成为 `index.md`。`config.ts` 中这两个选项已全局开启，新增文件夹
无需再改配置。
...
写完一篇就把该条改成指向章节的链接。这样路线图可见，但不会在站内搜索里
混入大量空页。章节文件本身**随写随建**——因为侧边栏自动生成，新建文件即自动出现，
无需改配置。

清单用**普通无序列表，不用 `- [ ]` 复选框语法**。原因：VitePress 1.6.4 不打包
markdown-it 的 task list 支持，`- [ ] xxx` 会渲染成字面文本 `[ ] xxx`；即便额外引入
task list 插件，渲染出的也是 `disabled` 的 input，永远点不动，属纯装饰。
改用普通列表后，**链接即已完成、纯文本即待写**，进度不需要图例就能看出来。
...
- 自定义主题、自定义 Vue 组件。用 VitePress 默认主题。
  **2026-09-09 更新**：此条被部分推翻——应用户要求做了深色科技感主题
  （`docs/.vitepress/theme/`：纯 CSS 变量覆盖 + hero 渐变/光晕，默认深色）。
  「无自定义 Vue 组件」仍然成立。
- 嵌套子目录（见上文阈值说明）。
